...
public class BRADLEYHTCJ48 {
	public static final int iterasyonSayisi=5;
	public static final String arffDosyaIsmi="dosya5likhtcj48";
	public static void main(String[] args) throws Exception {
		...
		ArrayList<Double> secerekAgacSayisi=new ArrayList<Double>();
		ArrayList<Double> secerekYaprakSayisi=new ArrayList<Double>();
		ArrayList<Double> secmedenAgacSayisi=new ArrayList<Double>();
		ArrayList<Double> secmedenYaprakSayisi=new ArrayList<Double>();
...
//		System.out.println("mrt_gecici size -> "+mrt_gecici.size());
...
//		System.out.println("walk_gecici size -> "+walk_gecici.size());
...
//		System.out.println("motor_gecici size -> "+motor_gecici.size());
...
//		System.out.println("train_gecici size -> "+train_gecici.size());
...
//		System.out.println("hsr_gecici size -> "+hsr_gecici.size());
...
//		System.out.println("run_gecici size -> "+run_gecici.size());
...
//		System.out.println("still_gecici size -> "+still_gecici.size());
		
		int mrt_artis=mrt_algoritmam.size()/iterasyonSayisi;
		int walk_artis=walk_algoritmam.size()/iterasyonSayisi;
		int car_artis=car_algoritmam.size()/iterasyonSayisi;
		int motor_artis=motor_algoritmam.size()/iterasyonSayisi;
		int train_artis=train_algoritmam.size()/iterasyonSayisi;
		int hsr_artis=hsr_algoritmam.size()/iterasyonSayisi;
		int run_artis=run_algoritmam.size()/iterasyonSayisi;
		int still_artis=still_algoritmam.size()/iterasyonSayisi;
		int bicycle_artis=still_algoritmam.size()/iterasyonSayisi;
		...
		for(int k=0;k<iterasyonSayisi;k++) {		
			...
			for(int i=walk_artis*k;i<k*walk_artis+walk_artis;i++) {
				double uzaklikMin=oklidUzaklikBul(walk_algoritmam_filtered.get(i), walkCentersOfClusters.get(0));
				double uzaklik=uzaklikMin;
				for(int j=1;j<walkIcin.numberOfClusters();j++) {
					if(oklidUzaklikBul(walk_algoritmam_filtered.get(i), walkCentersOfClusters.get(j))<uzaklikMin) {
						uzaklikMin=oklidUzaklikBul(walk_algoritmam_filtered.get(i), walkCentersOfClusters.get(j));
					}
					uzaklik+=oklidUzaklikBul(walk_algoritmam_filtered.get(i), walkCentersOfClusters.get(j));
				}
			...
			for(int i=mrt_artis*k;i<k*mrt_artis+mrt_artis;i++) {
				double uzaklikMin=oklidUzaklikBul(mrt_algoritmam_filtered.get(i), mrtCentersOfClusters.get(0));
				double uzaklik=uzaklikMin;
				for(int j=1;j<mrtIcin.numberOfClusters();j++) {
					if(oklidUzaklikBul(mrt_algoritmam_filtered.get(i), mrtCentersOfClusters.get(j))<uzaklikMin) {
						uzaklikMin=oklidUzaklikBul(mrt_algoritmam_filtered.get(i), mrtCentersOfClusters.get(j));
					}
					uzaklik+=oklidUzaklikBul(mrt_algoritmam_filtered.get(i), mrtCentersOfClusters.get(j));
				}
				double probCluster=uzaklikMin;
				double probClass=uzaklik;
			...
			for(int i=motor_artis*k;i<k*motor_artis+motor_artis;i++) {
				double uzaklikMin=oklidUzaklikBul(motor_algoritmam_filtered.get(i), motorCentersOfClusters.get(0));
				double uzaklik=uzaklikMin;
				for(int j=1;j<motorIcin.numberOfClusters();j++) {
					if(oklidUzaklikBul(motor_algoritmam_filtered.get(i), motorCentersOfClusters.get(j))<uzaklikMin) {
						uzaklikMin=oklidUzaklikBul(motor_algoritmam_filtered.get(i), motorCentersOfClusters.get(j));
					}
					uzaklik+=oklidUzaklikBul(motor_algoritmam_filtered.get(i), motorCentersOfClusters.get(j));
				}
				double probCluster=uzaklikMin;
				double probClass=uzaklik;
			...
			for(int i=train_artis*k;i<k*train_artis+train_artis;i++) {
				double uzaklikMin=oklidUzaklikBul(train_algoritmam_filtered.get(i), trainCentersOfClusters.get(0));
				double uzaklik=uzaklikMin;
				for(int j=1;j<trainIcin.numberOfClusters();j++) {
					if(oklidUzaklikBul(train_algoritmam_filtered.get(i), trainCentersOfClusters.get(j))<uzaklikMin) {
						uzaklikMin=oklidUzaklikBul(train_algoritmam_filtered.get(i), trainCentersOfClusters.get(j));
					}
					uzaklik+=oklidUzaklikBul(train_algoritmam_filtered.get(i), trainCentersOfClusters.get(j));
				}
				double probCluster=uzaklikMin;
				double probClass=uzaklik;
			...
			for(int i=bicycle_artis*k;i<k*bicycle_artis+bicycle_artis;i++) {
				double uzaklikMin=oklidUzaklikBul(bicycle_algoritmam_filtered.get(i), bicycleCentersOfClusters.get(0));
				double uzaklik=uzaklikMin;
				for(int j=1;j<bicycleIcin.numberOfClusters();j++) {
					if(oklidUzaklikBul(bicycle_algoritmam_filtered.get(i), bicycleCentersOfClusters.get(j))<uzaklikMin) {
						uzaklikMin=oklidUzaklikBul(bicycle_algoritmam_filtered.get(i), bicycleCentersOfClusters.get(j));
					}
					uzaklik+=oklidUzaklikBul(bicycle_algoritmam_filtered.get(i), bicycleCentersOfClusters.get(j));
				}
				double probCluster=uzaklikMin;
				double probClass=uzaklik;
			...
			for(int i=hsr_artis*k;i<k*hsr_artis+hsr_artis;i++) {
				double uzaklikMin=oklidUzaklikBul(hsr_algoritmam_filtered.get(i), hsrCentersOfClusters.get(0));
				double uzaklik=uzaklikMin;
				for(int j=1;j<hsrIcin.numberOfClusters();j++) {
					if(oklidUzaklikBul(hsr_algoritmam_filtered.get(i), hsrCentersOfClusters.get(j))<uzaklikMin) {
						uzaklikMin=oklidUzaklikBul(hsr_algoritmam_filtered.get(i), hsrCentersOfClusters.get(j));
					}
					uzaklik+=oklidUzaklikBul(hsr_algoritmam_filtered.get(i), hsrCentersOfClusters.get(j));
				}
				double probCluster=uzaklikMin;
				double probClass=uzaklik;
			...
			for(int i=run_artis*k;i<k*run_artis+run_artis;i++) {
				double uzaklikMin=oklidUzaklikBul(run_algoritmam_filtered.get(i), runCentersOfClusters.get(0));
				double uzaklik=uzaklikMin;
				for(int j=1;j<runIcin.numberOfClusters();j++) {
					if(oklidUzaklikBul(run_algoritmam_filtered.get(i), runCentersOfClusters.get(j))<uzaklikMin) {
						uzaklikMin=oklidUzaklikBul(run_algoritmam_filtered.get(i), runCentersOfClusters.get(j));
					}
					uzaklik+=oklidUzaklikBul(run_algoritmam_filtered.get(i), runCentersOfClusters.get(j));
				}
				double probCluster=uzaklikMin;
				double probClass=uzaklik;
			...
			for(int i=still_artis*k;i<k*still_artis+still_artis;i++) {
				double uzaklikMin=oklidUzaklikBul(still_algoritmam_filtered.get(i), stillCentersOfClusters.get(0));
				double uzaklik=uzaklikMin;
				for(int j=1;j<stillIcin.numberOfClusters();j++) {
					if(oklidUzaklikBul(still_algoritmam_filtered.get(i), stillCentersOfClusters.get(j))<uzaklikMin) {
						uzaklikMin=oklidUzaklikBul(still_algoritmam_filtered.get(i), stillCentersOfClusters.get(j));
					}
					uzaklik+=oklidUzaklikBul(still_algoritmam_filtered.get(i), stillCentersOfClusters.get(j));
				}
				double probCluster=uzaklikMin;
				double probClass=uzaklik;
...
//			System.out.println("bicycle_cluster_Threshold -> "+bicycle_cluster_threshold);
//			
...
//			System.out.println("still_cluster_Threshold -> "+still_cluster_threshold);
			...
			for(int i=walk_artis*k;i<k*walk_artis+walk_artis;i++) {
				...
				if(walkProbClasses.get(i)<walk_class_threshold && walkProbClusters.get(i)>walk_cluster_threshold) {
					walk_train.add(walk_algoritmam.get(i));
					training_data.add(walk_algoritmam.get(i));
				}else if(walkProbClasses.get(i)>walk_class_threshold && walkProbClusters.get(i)>walk_cluster_threshold) {
					walk_train.add(walk_algoritmam.get(i));
					training_data.add(walk_algoritmam.get(i));
				}
				secmedenTrain.add(walk_algoritmam.get(i));
			}
			
			for(int i=mrt_artis*k;i<k*mrt_artis+mrt_artis;i++) {
				if(mrtProbClasses.get(i)<mrt_class_threshold && mrtProbClusters.get(i)>mrt_cluster_threshold) {
					mrt_train.add(mrt_algoritmam.get(i));
					training_data.add(mrt_algoritmam.get(i));
				}else if(mrtProbClasses.get(i)>mrt_class_threshold && mrtProbClusters.get(i)>mrt_cluster_threshold) {
					mrt_train.add(mrt_algoritmam.get(i));
					training_data.add(mrt_algoritmam.get(i));
				}
				secmedenTrain.add(mrt_algoritmam.get(i));
			...
			for(int i=motor_artis*k;i<k*motor_artis+motor_artis;i++) {
				if(motorProbClasses.get(i)<motor_class_threshold && motorProbClusters.get(i)>motor_cluster_threshold) {
					motor_train.add(motor_algoritmam.get(i));
					training_data.add(motor_algoritmam.get(i));
				}else if(motorProbClasses.get(i)>motor_class_threshold && motorProbClusters.get(i)>motor_cluster_threshold) {
					motor_train.add(motor_algoritmam.get(i));
					training_data.add(motor_algoritmam.get(i));
				}
				secmedenTrain.add(motor_algoritmam.get(i));

			}
			
			for(int i=train_artis*k;i<k*train_artis+train_artis;i++) {
				if(trainProbClasses.get(i)<train_class_threshold && trainProbClusters.get(i)>train_cluster_threshold) {
					train_train.add(train_algoritmam.get(i));
					training_data.add(train_algoritmam.get(i));
				}else if(trainProbClasses.get(i)>train_class_threshold && trainProbClusters.get(i)>train_cluster_threshold) {
					train_train.add(train_algoritmam.get(i));
					training_data.add(train_algoritmam.get(i));
				}
				secmedenTrain.add(train_algoritmam.get(i));

			}
			
			for(int i=bicycle_artis*k;i<k*bicycle_artis+bicycle_artis;i++) {
				if(bicycleProbClasses.get(i)<bicycle_class_threshold && bicycleProbClusters.get(i)>bicycle_cluster_threshold) {
					bicycle_train.add(bicycle_algoritmam.get(i));
					training_data.add(bicycle_algoritmam.get(i));
				}else if(bicycleProbClasses.get(i)>bicycle_class_threshold && bicycleProbClusters.get(i)>bicycle_cluster_threshold) {
					bicycle_train.add(bicycle_algoritmam.get(i));
					training_data.add(bicycle_algoritmam.get(i));
				}
				secmedenTrain.add(bicycle_algoritmam.get(i));

			}
			
			for(int i=hsr_artis*k;i<k*hsr_artis+hsr_artis;i++) {
				if(hsrProbClasses.get(i)<hsr_class_threshold && hsrProbClusters.get(i)>hsr_cluster_threshold) {
					hsr_train.add(hsr_algoritmam.get(i));
					training_data.add(hsr_algoritmam.get(i));
				}else if(hsrProbClasses.get(i)>hsr_class_threshold && hsrProbClusters.get(i)>hsr_cluster_threshold) {
					hsr_train.add(hsr_algoritmam.get(i));
					training_data.add(hsr_algoritmam.get(i));
				}
				secmedenTrain.add(hsr_algoritmam.get(i));

			}	
			
			for(int i=run_artis*k;i<k*run_artis+run_artis;i++) {
				if(runProbClasses.get(i)<run_class_threshold && runProbClusters.get(i)>run_cluster_threshold) {
					run_train.add(run_algoritmam.get(i));
					training_data.add(run_algoritmam.get(i));
				}else if(runProbClasses.get(i)>run_class_threshold && runProbClusters.get(i)>run_cluster_threshold) {
					run_train.add(run_algoritmam.get(i));
					training_data.add(run_algoritmam.get(i));
				}
				secmedenTrain.add(run_algoritmam.get(i));

			}
			
			for(int i=still_artis*k;i<k*still_artis+still_artis;i++) {
				if(stillProbClasses.get(i)<still_class_threshold && stillProbClusters.get(i)>still_cluster_threshold) {
					still_train.add(still_algoritmam.get(i));
					training_data.add(still_algoritmam.get(i));
				}else if(stillProbClasses.get(i)>still_class_threshold && stillProbClusters.get(i)>still_cluster_threshold) {
					still_train.add(still_algoritmam.get(i));
					training_data.add(still_algoritmam.get(i));
				}
				secmedenTrain.add(still_algoritmam.get(i));
			...
			secerekAgacSayisi.add((rf.measureTreeSize()));
			secerekYaprakSayisi.add((rf.measureNumLeaves()));
			...
			J48 rf2=new J48();
			BufferedWriter secmedendosya=new BufferedWriter(new FileWriter("./"+arffDosyaIsmi+"/secmeden"+k+".arff"));
			...
			secmedenAgacSayisi.add((rf2.measureTreeSize()));
			secmedenYaprakSayisi.add((rf2.measureNumLeaves()));
		...
		System.out.println("Seçerek Agac Sayisi");
		for(int tt=0;tt<secerekAgacSayisi.size();tt++) {
			System.out.println(secerekAgacSayisi.get(tt));
		}
		System.out.println("Seçmeden Agac Sayisi");
		for(int tt=0;tt<secmedenAgacSayisi.size();tt++) {
			System.out.println(secmedenAgacSayisi.get(tt));
		}	
		System.out.println("Seçerek Yaprak Sayisi");
		for(int tt=0;tt<secerekYaprakSayisi.size();tt++) {
			System.out.println(secerekYaprakSayisi.get(tt));
		}
		System.out.println("Seçmeden Yaprak Sayisi");
		for(int tt=0;tt<secmedenYaprakSayisi.size();tt++) {
			System.out.println(secmedenYaprakSayisi.get(tt));
		}			
